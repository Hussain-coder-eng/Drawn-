...
/** Evenly spaced samples by index along a polyline; caps stage-script size (one Gemini stage per edge). */
export function resamplePolylinePoints(points: Point[], maxPoints: number): Point[] {
  if (points.length <= maxPoints || points.length < 2 || maxPoints < 2) return points;
  const out: Point[] = [];
